...
def give_exam(dataset, batch_size=32, max_batches=-1):
    ...
    for b, (x, y) in enumerate(loader):
        x = x.to(model.device)
        d1d2 = x[:, :args.ndigits * 2]  # A list comprising the first two numbers, `ndigit` digits each.
        d1d2d3 = sample(model, d1d2, args.ndigits + 1)  # Predict the next number, at most `ndigit + 1` digits.
...
def give_exam_generalised(dataset, seqlen=2, batch_size=32, max_batches=-1):
    ...
    for b, (x, y) in enumerate(loader):
        x = x.to(model.device)
        input_sequence = x[:, :seqlen * args.ndigits]  # The first three numbers, the second of which is a decoy.
